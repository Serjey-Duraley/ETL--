...
def fix_types(df: pd.DataFrame) -> pd.DataFrame:
    """
    1. Удаляет константные столбцы.
    2. Приводит числовые поля к int.
    3. Парсит published_at в datetime.
    4. Конвертирует duration из ISO 8601 в секунды.
    5. Добавляет текстовую категорию category_name.
    """
    df = df.copy()
    # Удаление констант
...
def handle_missing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Проверяем пропуски в числовых столбцах
...
def merge_channel_stats(videos: pd.DataFrame, channel_stats: pd.DataFrame) -> pd.DataFrame:
    """
    Объединяет видео со статистикой каналов через LEFT JOIN.
    """
    n_before = len(videos)
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Создаёт новые признаки для ML-модели.
    """
    df = df.copy()
    ...
    df['title_upper_ratio'] = df['title'].apply(
        lambda t: sum(1 for c in t if c.isupper()) / max(len(str(t)), 1)
    )
    ...
    # --- Признаки канала (из YouTube API) ---
    df['log_subscribers'] = np.log1p(df.get('subscriber_count', 0))
