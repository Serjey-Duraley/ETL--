...
def fetch_channel_stats(channel_ids: list, api_key: str) -> pd.DataFrame: 
    ...
    ids_joined = ','.join(channel_ids)  # передаём список через запятую 
 
    params = { 
        'part':       'statistics',  # запрашиваем только блок статистики 
        'id':         ids_joined, 
        'key':        api_key, 
        'maxResults': 50, 
    } 
    ...
    for item in items: 
        ...
        records.append({ 
            'channel_id':          item['id'], 
            # hiddenSubscriberCount=True означает скрытые подписчики -> 0 
...
def fetch_channel_stats_mock() -> pd.DataFrame: 
    ...
    data = [ 
        ...
        ('UCBcRF18a7Qf58cCRy5xuWwQ', 6_300_000,    900_000_000, 430),  # MrBeast (пример) 
        # ... добавьте реальные данные для всех 15 каналов из датасета 
]
